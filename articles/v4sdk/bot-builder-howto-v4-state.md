...
在 `dialogs/mainDialog/index.js` 文件中更新构造函数，使之接受 `userState` 作为第二个参数。 然后从 `conversationState` 创建 `topicStates` 属性，从 `userState` 创建 `userProfile` 属性。
