...
本主题的其余部分在 Echo 机器人的基础上进行构建。 用于创建此项目的 Echo 机器人示例代码可以在 [C# 示例](https://aka.ms/cs-echobot-sample)或 [JS 示例](https://aka.ms/js-echobot-sample)中找到。 可以使用 Bot Framework Emulator 连接到机器人，与之聊天并对其进行测试。 下面的示例将来自用户的每个消息添加到列表。 然后会将包含列表的数据结构保存到存储中。
