...
在 **CustomPromptBot.cs** 中获取状态属性并调用 helper 方法。 （注意，`_accessors` 实例属性在机器人的构造函数中设置。）
...
在 **bot.js** 中获取状态属性并调用 helper 方法。 （注意，`conversationFlow` 实例属性在机器人的构造函数中设置。）
