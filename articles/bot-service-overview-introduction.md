...
Azure 机器人服务提供的工具可用于在一个位置构建、测试、部署和管理智能机器人。 通过 SDK 提供的模块化可扩展框架，开发人员能够利用模板来创建可提供语音、自然语言识别、问题和答案处理等功能的机器人。  

## <a name="what-is-a-bot"></a>什么是机器人？

机器人是可以通过聊天来与人类用户通信的应用。 该软件可通过文本、语音、图形或菜单来与用户交互，并执行聊天相关的任务。 这种交互可能是简单的问答式交流。复杂的机器人可让我们以自然方式与服务交互，在幕后，机器人利用与现有服务妥善集成的智能技术。

系统可以通过机器人收集信息，或者向用户提供一种不太像是计算机操作，而更像是交互的用户体验。 不需要直接与人类交互时，机器人还能将简单的任务（例如订餐，或者从用户收集个人资料信息）转移到系统（或者与其他系统的集成）。 
...
在编写代码之前，请参阅[设计指南](bot-service-design-principles.md) ，了解最佳做法并确定机器人的需求。 可以创建简单的机器人或者包含更复杂的功能，例如语音、自然语言识别或回答用户的提问。 

### <a name="build"></a>构建

机器人是一项 Web 服务，可实现聊天式界面并与机器人服务通信。 可在任意数目的环境和语言中创建此解决方案，我们会提供简易入门模板。 可以在 [Azure 门户](bot-service-quickstart.md)中开始进行机器人开发，或者使用下面所列的模板，以所选的语言进行本地开发。

| .NET 模板 | JavaScript 模板 | 
| --- | --- | 
...
| 管理多个模型 | 如果使用多个模型（例如 LUIS 和 QnA Maker），在与机器人聊天过程中，机器人能够明智地确定何时使用哪个模型 | [Dispatch 工具](~/v4sdk/bot-builder-tutorial-dispatch.md) |
| 添加卡片和按钮 | 使用除文本以外的媒体（例如图形、菜单和卡片）来增强用户体验 | [如何添加卡片](v4sdk/bot-builder-howto-add-media-attachments.md) |

> [!NOTE]
> 上面的表格不是完整的列表。 浏览左侧的文章，从[发送消息](~/v4sdk/bot-builder-howto-send-messages.md)开始，了解更多机器人功能。

此外，我们提供 CLI 工具来帮助你创建、管理和测试机器人资产。 这些工具可以通过命令行管理机器人配置文件、LUIS 应用、QnA 知识库、模拟聊天，等等。 可在[自述文件](https://github.com/Microsoft/botbuilder-tools/blob/master/README.md)中找到更多详细信息。
...
### <a name="publish"></a>发布 
准备就绪后，[将机器人发布到 Azure](bot-builder-howto-deploy-azure.md) 或自己的 Web 服务或数据中心。
...
了解如何[收集分析数据](bot-service-manage-analytics.md)。
